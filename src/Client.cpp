...
etcd::Client::Client(std::string const & address)
  : client(utility::conversions::to_utf16string(address))
{
}
...
pplx::task<etcd::Response> etcd::Client::send_put_request(web::http::uri_builder & uri, std::string const & key, std::string const & value)
{
  ...
  return Response::create(client.request(web::http::methods::PUT, uri.to_string(), utility::conversions::to_utf16string(data)
	  , utility::conversions::to_utf16string(content_type)));
}

pplx::task<etcd::Response> etcd::Client::get(std::string const & key)
{
  web::http::uri_builder uri(utility::conversions::to_utf16string("/v2/keys" + key));
...
{
  web::http::uri_builder uri(utility::conversions::to_utf16string("/v2/keys" + key));
  uri.append_query(U("prevValue"), utility::conversions::to_utf16string(old_value));
  return send_put_request(uri, "value", value);
}
